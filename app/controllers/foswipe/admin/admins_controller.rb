class Foswipe::Admin::AdminsController < Foswipe::ApplicationController
  ...
  def destroy
    @admin.destroy
    respond_to do |format|
      format.html { redirect_to admin_admins_url, notice: 'Admin was successfully deleted.' }
      format.json { head :no_content }
    end
  end
  ...
  def assign_tickets_to_supports
    ...
    respond_to do |format|
      format.html { redirect_to admin_tickets_url, notice: 'Successfully assigned to support team.' }
      format.json { head :no_content }
    end
  end
